...
{									\
  ...
  {									\
      ...
      case 2:	{								\
        ...
        int offset_idx_x = lattice_1_1 * loop_j_stride;		\
									\
	      if (loop_i_ub > loop_i_lb && loop_j_ub > loop_j_lb &&	loop_i_stride > 0 && loop_j_stride > 0)	{								\
		      if (loop_j_stride == 1 && loop_j_ub > 1 && loop_j_lb == 0) { \
            for (int i = loop_i_lb; i <= loop_i_ub; i += loop_i_stride) {							\
              idx_y = orig.coordinates[0] + i * lattice_0_0;	\
              idx_x = orig.coordinates[1] + i * lattice_0_1;	\
            ...
            for (int i = loop_i_lb; i <= loop_i_ub; i += loop_i_stride)	{							\
              idx_y = orig.coordinates[0] + i * lattice_0_0	+ lattice_1_0 * loop_j_lb;			\
              idx_x = orig.coordinates[1] + i * lattice_0_1	+ lattice_1_1 * loop_j_lb;			\
              for (int j = loop_j_lb; j <= loop_j_ub; j += loop_j_stride) {						\
                /* Compute. */				\
                datatypename A_val = data_vector[a_data_pos++]; \
                for (int k = Tj; k < TjUB; ++k)		\
  ...
  if (!bypass_codelets){
    if (dim_o == 1)
    {
      if (data_is_float) {
        ...
        for (int i = 0; i < nb_origins; ++i) {
          s_origin_1d_t orig = ((s_origin_1d_t*)spf_matrix->origins_list.origins)[i];
    ...
    else if (dim_o == 2)
    {
      if (data_is_float) {
        #pragma omp parallel for num_threads(nThreads)
        for (int i = 0; i < nb_origins; ++i) {
          s_origin_2d_t orig = ((s_origin_2d_t*)spf_matrix->origins_list.origins)[i];
      ...
      exit (156);
	    ...
	    if (dim_o == 2) {
          ...
          for (int Tj = 0; Tj < nj; Tj += TJ) {
            ...
            for (int i = 0; i < nb_origins; ++i) {
		          s_origin_2d_t orig = ((s_origin_2d_t*)spf_matrix->origins_list.origins)[i];
		          run_shape_o2d_mm_float(spf_matrix, orig, B, C, ni, nj, nk, Tj, loop_ub);
		        }
		      }
	      } else {
          #pragma omp parallel for
          for (int Tj = 0; Tj < nj; Tj += TJ) {
            ...
            for (int i = 0; i < nb_origins; ++i) {
		          s_origin_2d_t orig = ((s_origin_2d_t*)spf_matrix->origins_list.origins)[i];
		          run_shape_o2d_mm_double(spf_matrix, orig, B, C, ni, nj, nk, Tj, loop_ub);
		        }
		      }
	      }
	    }
