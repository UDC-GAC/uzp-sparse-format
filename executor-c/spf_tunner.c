...
#define DEBUG 0
int GLOBAL_GROUPING_POINTS_LIMIT = -1;

// Enum for tuning options
typedef enum {
    FIND_MERGE_CONSECUTIVE_ORIGINS = 1,
    REORDER_ORIGINS_FAVOUR_LOCALITY = 2,
    SHUFFLE_ORDER_OF_ORIGINS = 3
} TunerMode;
...
int compare_coordinates_xsort(const void* a, const void* b) {
    ...
    const int block_size = GLOBAL_GROUPING_POINTS_LIMIT;
...
void shuffle_origins(ShapeMap* new_map, ShapeOrigins* shape, int* hashmap_to_merge) {
...
void merge_shapes(ShapeMap* new_map, ShapeOrigins* shape, int* hashmap_to_merge) {
    ...
    for (int j = shape->count - 1; j >= 0; --j)
    {
        if (hashmap_to_merge[j] != -1) {
          ...
          if(((shape->coordinates[j].data_count + shape->coordinates[merge_idx].data_count)*(shape->end_vertex + 1)) < GLOBAL_GROUPING_POINTS_LIMIT){
...
s_spf_structure_t* spf_uzp_optimizer(s_spf_structure_t* spf_matrix, TunerMode tuner_mode)
{
  if (!spf_matrix) {
    if (DEBUG) {
  ...
  ShapeMap new_map;
  new_map.count = 0;
  new_map.shapes = (void **)malloc(4000 * sizeof(void *));  // Allocate memory once for all shapes
  new_map.capacity = 4000;
  ...
  for (int i = 0; i < origin_map.count; ++i) {
    ...
    if(tuner_mode == FIND_MERGE_CONSECUTIVE_ORIGINS){
      ...
      for (int j = 0; j < shape->count; ++j) {
        ...
        // if(((shape->coordinates[j].y / GROUPING_POINTS_LIMIT) == (next_coordinate_row / GROUPING_POINTS_LIMIT)) && ((shape->coordinates[j].x / GROUPING_POINTS_LIMIT) == (next_coordinate_column / GROUPING_POINTS_LIMIT))){
    ...
    if(tuner_mode == REORDER_ORIGINS_FAVOUR_LOCALITY){
      sort_origins_to_favour_locality(&new_map, shape, hashmap_to_merge);
    }
    if(tuner_mode == SHUFFLE_ORDER_OF_ORIGINS){
      shuffle_origins(&new_map, shape, hashmap_to_merge);
    }
...
void usage(char** argv) {
    fprintf(stderr, "Usage: %s <src_path> <dst_path> <grouping_point_limit> <tuner_mode>\n", argv[0]);
    fprintf(stderr, "Tuner Mode:\n");
    fprintf(stderr, "  1 - FIND_MERGE_CONSECUTIVE_ORIGINS\n");
    fprintf(stderr, "  2 - REORDER_ORIGINS_FAVOUR_LOCALITY\n");
    fprintf(stderr, "  3 - SHUFFLE_ORDER_OF_ORIGINS\n");
    exit(1);
}
...
int main(int argc, char** argv) {
    ...
    GLOBAL_GROUPING_POINTS_LIMIT = block_size;
    int tuner_mode = atoi(argv[4]);

    // Validate block_size
    if (block_size <= 0) {
        fprintf(stderr, "[ERROR] Invalid grouping point limit: %s. Must be a positive integer.\n", argv[3]);
        return 1;
    }

    if (tuner_mode < 1 || tuner_mode > 3) {
        fprintf(stderr, "[ERROR] Invalid tuner mode: %s. Must be 1, 2, or 3.\n", argv[4]);
        usage(argv);
    }

    printf("Source Path: %s\n", src_path);
    printf("Destination Path: %s\n", dst_path);
    printf("Grouping Points Limit: %d\n", block_size);
    printf("Selected Tuner Mode: %d\n", tuner_mode);
    ...
    if (file_size != -1) {
        ...
        fprintf(stderr, "[ERROR] Unable to retrieve file size for: %s\n", src_path);
        return 1;
    }

    // Load the matrix
    s_spf_structure_t* spf_mat = spf_matrix_read_from_file(src_path);
    if (!spf_mat) {
        fprintf(stderr, "[ERROR] Failed to read matrix from file: %s\n", src_path);
        return 1;
    }
    
    spf_mat = spf_uzp_optimizer(spf_mat, tuner_mode);

    // Construct final output path
    char final_dst_path[2048];
    // snprintf(final_dst_path, sizeof(final_dst_path), "%s_grp_till_%d_mode_%d.uzp", dst_path, block_size, tuner_mode);

    // Write the processed matrix to a file
    ...
    printf("The processing completed. Output saved at: %s\n", dst_path);
    return 0;
}
